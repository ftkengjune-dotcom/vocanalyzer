      ...
      input: [
        {
          role: "system",
          content:
            "You are a precise English morphology & etymology tutor. Return JSON only.",
        },
        {
          role: "user",
          content: `Analyze the English word strictly as JSON with this schema:
{
  "word": string,
...
- Output JSON only (NO code fences).`,
    ...
    // 모델이 JSON을 안 주는 경우도 대비 → 파싱 시도
    let obj;
    try {
      obj = JSON.parse(raw);
    } catch {
      // 최소 스키마로 감싸서 JSON은 항상 보장
      obj = { raw, error: "Model did not return strict JSON" };
    }
